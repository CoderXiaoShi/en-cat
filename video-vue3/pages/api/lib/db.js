...
async function initDb() {
  ...
  if (videoCount === 0) {
    database.run(`INSERT INTO videos (title, url, subtitle_url, duration) VALUES (?, ?, ?, ?)`, 
      ['示例英语教程', '/videos/sample.mp4', '/subtitles/sample.vtt', 300]);
    database.run(`INSERT INTO notebooks (title, video_id) VALUES (?, ?)`, 
      ['我的学习笔记', 1]);
    database.run(`INSERT INTO notes (notebook_id, content, audio_url, audio_duration, timestamp) VALUES (?, ?, ?, ?, ?)`, 
...
module.exports = { getDb, initDb, closeDb, saveDb, queryOne, queryAll, run };
